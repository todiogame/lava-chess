...
function riseLava(og, cell, spell, casterEntity, targetEntity) {
  ...
  cellM.aoe = cellM.aoe.filter(a => !a.permanent); //remove any permanent aoe. We will rework if we ad a character that can survive lava
  cellM.floor = false;

  //cast again on cells completely surrounded by lava
  og.map.forEach(h => {
    if (h.floor && utils.completelySurroundedByLava(h, og)) {
      let newtarget = utils.findEntityOnCell(h, og)
      riseLava(og, h, spell, casterEntity, newtarget)
    }
  })
}
  ...
  if (summonData && !targetEntity) {
    ...
    if (summonData.isUnique) {
      let previous = og.entities.find(
        (e) => e.id == summonData.id && e.summoner == casterEntity,
      );
...
function mage_force(og, cell, spell, casterEntity, targetEntity, direction, mainCell) {
  if (typeof window != 'undefined' && window.document) {
    Anim.flashParticles(casterEntity.pos);
  }
...
function fisherman_push(og, cell, spell, casterEntity, targetEntity) {
  if (targetEntity && !targetEntity.isEthereal()) damage(og, cell, spell, casterEntity, targetEntity);
  push(og, cell, spell, casterEntity, targetEntity);
}
...
function ninja_slash(og, cell, spell, casterEntity, targetEntity, direction, mainCell) {
  if (typeof window != 'undefined' && window.document) {
    if (casterEntity.pos.add(direction).equals(cell)) { //little hack to cast the anim only once
      ...
      if(shadow) Anim.shadowParticles(shadow.pos);
    }
  }
  if (targetEntity && !targetEntity.isEthereal()) damage(og, cell, spell, casterEntity, targetEntity);
}
...
function zombie_crawl(og, cell, spell, casterEntity, targetEntity) {
  ...
  if (targetEntity && !targetEntity.isEthereal()) {
    damage(og, cell, spell, casterEntity, targetEntity);
...
function warrior_charge(og, cell, spell, casterEntity, targetEntity) {
  if (targetEntity) {
    if (!targetEntity.isEthereal()) damage(og, cell, spell, casterEntity, targetEntity);
...
function ethereal_form(og, cell, spell, casterEntity, targetEntity,) {
  if (targetEntity) {
    buffPM(og, cell, spell, casterEntity, targetEntity)
    targetEntity.auras.push({ name: "ethereal", ttl: 1 });
    if (typeof window != 'undefined' && window.document)
      Anim.flashParticles(cell);
  }
}
