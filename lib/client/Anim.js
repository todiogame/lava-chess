...
module.exports = class Anim {
  ...
  static lavaParticles(cell) {
    ...
    const bubbles = 30;
    const canvasX = document.getElementById("canvas").height;
    for (var i = 0; i < bubbles; i++) {
      let particle = {
        xOrigin: coords.x,
        yOrigin: coords.y,
        x:  r(coords.x+0.045*canvasX,coords.x-0.045*canvasX),
        y:  r(coords.y+0.055*canvasX,coords.y-0.055*canvasX),
        radius: r(15),
        color: colors[Math.floor(Math.random() * colors.length)],
        rotation: r(0, 360, true),
        speed: r(0.1,0.5),
        ...
        moving: true,
        randomPos: true,
        randomx: ()=> r(coords.x+0.045*canvasX,coords.x-0.045*canvasX),
        randomy: ()=> r(coords.y+0.055*canvasX,coords.y-0.055*canvasX),
      };
      setTimeout(() => {
        particle.moving = false;
      }, 700);
