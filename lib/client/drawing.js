...
function drawAOEtips(aoes) {
    ...
    ctx.beginPath();
    if (ctx.roundRect) // ca bug firefox
        ctx.roundRect(layout.hexToPixel(hoverInfo.cell).x - width / 2, layout.hexToPixel(hoverInfo.cell).y - 90 - 20 * aoes.length, width, 10 + 20 * aoes.length, 20);
    else
        ctx.fillRect(layout.hexToPixel(hoverInfo.cell).x - width / 2, layout.hexToPixel(hoverInfo.cell).y - 90 - 20 * aoes.length, width, 10 + 20 * aoes.length, 20);
...
function drawHPBar(entity) {
    ...
    ctx.beginPath();
    if (ctx.roundRect) // ca bug firefox
        ctx.roundRect(layout.hexToPixel(hoverInfo.cell).x - 25, layout.hexToPixel(hoverInfo.cell).y - 75, 50, 20, 20);
    else
        ctx.fillRect(layout.hexToPixel(hoverInfo.cell).x - 25, layout.hexToPixel(hoverInfo.cell).y - 75, 50, 20, 20);
    ...
    for (let i = 0; i < entity.maxHP; i++) {
        ...
        ctx.fillRect(layout.hexToPixel(hoverInfo.cell).x - 20 + i * 10, layout.hexToPixel(hoverInfo.cell).y - 70, 10, 10);
    }
}
...
function highlightGlyphs(entity){
    let listCells = map.filter(h => h.aoe.length && h.aoe.some(eff => eff.source?.entity == entity))
}
