...
function drawPerso(entity, og) {
  ...
  if (entity.isEthereal()) ctx.globalAlpha = 1
...
function displayPickCharacterHUD(og) {
  ...
  ctx.fillStyle = og.currentTeam;
  ctx.fillText(
    og.currentTeam == TEAM ? 'Your turn' : "Enemy's turn" + ' to ' +
  ...
  if (currentEntity) {
    ...
    ctx.fillText(currentEntity.name, 20, 100);

    // Draw character title
    ctx.font = '24px "Russo One", sans-serif';
    ctx.fillStyle = '#ff6600';
    ctx.fillText(currentEntity.title, 20, 140);

    // Draw character difficulty stars
    ctx.fillStyle = '#ff6600';
    ctx.fillText('Difficulty: ' + displayStars(currentEntity.difficulty), 20, 170);
    ...
    for (let i = 0; i < currentEntity.spellsDisplay.length; i++) {
      ...
      if (spell) {
        ...
        ctx.fillStyle = '#ffc107';
        ctx.fillText(spell.description, 20, 310 + i * 60);
      }
    }
  }
}
