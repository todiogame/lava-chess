...
function loadImages(entities) {
  // Load the buttonSpell imagesSpells
  const imagesSpells = [];
  entities.forEach(e => {
    imagesSpells[e.name] = []
    for (let i = 0; i < 3; i++) {
      const image = new Image();
      image.src = `./pics/spells/${e.name.toLowerCase()}_${i + 1}.png`;
      imagesSpells[e.name].push(image);
    }
  })
  return imagesSpells;
}
...
function drawSpells() {
  ...
  for (let i = 0; i < 3; i++) {
    const x = 10 + i * (buttonSpell.width + 10);
    const y = canvas.height - buttonSpell.height - 30;

    const spellImage = imagesSpells[currentPlayer.entity.name][i]
...
function drawClock() {
  ...
  ctx.drawImage(clockImage, 610, 10, SIZE_PERSO, SIZE_PERSO);
  ctx.font = "bold 50px 'Russo One', sans-serif";
  ctx.fillStyle = "orange";
  ctx.fillText(timeleft, 623 + (timeleft <= 9 ? 13 : 0), 65);
  // }
}
...
function drawAOEtips(aoes) {
  ...
  if (ctx.roundRect)
    ...
    ctx.roundRect(
      ...
      layout.hexToPixel(hoverInfo.cell).y - 90 - 20 * aoes.length,
      width,
      10 + 20 * aoes.length,
      20,
    );
  ...
  for (let i = 0; i < aoes.length; i++) {
    ...
    ctx.fillText(
      aoes[i].name,
      layout.hexToPixel(hoverInfo.cell).x -
      ctx.measureText(aoes[i].name).width / 2,
...
function drawHPBar(entity) {
  ...
  if (ctx.roundRect)
    // ca bug firefox
    ctx.roundRect(
      layout.hexToPixel(hoverInfo.cell).x - 25,
      layout.hexToPixel(hoverInfo.cell).y - 75,
      50,
      20,
      20,
    );
  ...
  for (let i = 0; i < entity.maxHP; i++) {
    ...
    ctx.fillRect(
      layout.hexToPixel(hoverInfo.cell).x - 20 + i * 10,
      layout.hexToPixel(hoverInfo.cell).y - 70,
      10,
      10,
    );
  }
