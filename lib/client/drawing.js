...
function drawSpells() {
  //draw spell buttons
  for (let i = 0; i < 3; i++) {
    const x = buttonSpell.w_offset + i * (buttonSpell.width + 10);
    const y = buttonSpell.h_offset - buttonSpell.height;
...
function drawHPBar(entity) {
  ...
  for (let i = 0; i < entity.maxHP; i++) {
    ...
    ctx.fillRect(
      ...
      layout.hexToPixel(hoverInfo.cell).y - SIZE_PERSO + 5,
      10,
      10,
    );
    ctx.strokeRect(layout.hexToPixel(hoverInfo.cell).x - 20 + i * 10,
      layout.hexToPixel(hoverInfo.cell).y - SIZE_PERSO + 5,
      10,
      10);
  }
