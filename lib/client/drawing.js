...
function drawMap(og) {
  ...
  if (og.currentPlayer) drawSpells(og, buttonSpell2, og.currentPlayer, false) //top left
}
...
function drawSpells(og, buttonSpell, player, bottomRow,) {
  ...
  if (bottomRow || player.entity.team != TEAM) { //dont draw top row spells if we are playing
    //draw move button
    let x = buttonSpell.w_offset;
    let y = buttonSpell.h_offset - buttonSpell.height;
    ...
    for (let i = 0; i < player.spells.length; i++) {
      ...
      const spellImage = player.spells[i].image;
  ...
  if (bottomRow) {
    ...
    if (hoverInfo.element) {
      ...
      y = 490
    } else y = buttonSpell.h_offset - buttonSpell.height - 130
  } else {
    y = 50
  }
  drawPlayingEntityTopLeft(player.entity, buttonSpell.w_offset, y, SIZE_PERSO * (bottomRow ? 1 : 2));
