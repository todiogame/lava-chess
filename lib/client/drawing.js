...
function drawPerso(entity) {
  ...
  //draw hp bar : 
  if (!isPickPhase && entity?.maxHP && displayAllHP) {
    drawHPBar(entity);
  }
}
...
function drawHPBar(entity) {
  ...
  if (ctx.roundRect)
    // ca bug firefox
    ctx.roundRect(
      layout.hexToPixel(entity.pos).x - 50,
      layout.hexToPixel(entity.pos).y - SIZE_PERSO,
      100,
      40,
      20,
    );
  ...
  for (let i = 0; i < entity.maxHP; i++) {
    ...
    ctx.fillRect(
      layout.hexToPixel(entity.pos).x - 40 + i * 20,
      layout.hexToPixel(entity.pos).y - SIZE_PERSO + 10,
      20,
      20,
    );
    ctx.strokeRect(layout.hexToPixel(entity.pos).x - 40 + i * 20,
      layout.hexToPixel(entity.pos).y - SIZE_PERSO + 10,
      20,
      20);
  }
