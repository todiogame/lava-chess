...
const SPAWN_BLUE = "rgb(0,200,200, 0.7)";
const SPAWN_RED = "rgb(255, 0, 0, 0.7)";
...
function drawPerso(entity) {
    ...
    //draw shield
    if (entity.armor) {
        console.log(entity.shieldColor)
        ctx.beginPath();
        ctx.arc(pPerso.x, pPerso.y - (SIZE_PERSO) / 4, 50, 0, 2 * Math.PI);
        if (entity.shieldColor) ctx.fillStyle = ARRAY_GLYPH_COLOR[entity.shieldColor]
