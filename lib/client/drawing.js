...
function drawPerso(entity) {
  ...
  //draw hp bar : 
  if (!isPickPhase && entity?.maxHP && (displayAllHP || hoverInfo.help)) {
...
function drawMap(og) {
  ...
  if (og.map) {
    ...
    if (og.currentPlayer) drawPlayingEntityTopLeft(og.currentPlayer.entity);
  ...
  if (og.currentPlayer) drawSpells(og)
}

function drawHelpCircle() {
  ctx.globalAlpha = 0.5;
  ctx.drawImage(helpCircleImage, c.CANVAS.WIDTH - 330, 10, 150, 150);
  ctx.globalAlpha = 1;
}
...
function drawSpells(og) {
  ...
  if (og.currentPlayer.movePoint <= 0) {
  ...
  for (let i = 0; i < og.currentPlayer.spells.length; i++) {
    ...
    const spellImage = og.currentPlayer.spells[i].image;
    ...
    if (og.currentPlayer.spells[i].passive) {
      ctx.fillStyle = colorSpellPassive;
      ctx.fillRect(x, y, buttonSpell.width, buttonSpell.height);
    } else if (og.currentPlayer.entity.auras.length && og.currentPlayer.entity.auras.some(a => a.name == "silence")) {
      ctx.drawImage(silenceIcon, x, y, buttonSpell.width, buttonSpell.height);
      ctx.fillStyle = colorSpellSilenced;
      ctx.fillRect(x, y, buttonSpell.width, buttonSpell.height);
    } else if (og.currentPlayer.spells[i].currentCD > 0) {
      ...
      ctx.fillStyle = "white";
      ctx.fillText(
        og.currentPlayer.spells[i].currentCD,
        x + 35, y + 70,
      );
    }
  }
...
function drawClock() {
  let timeleft;
  if (globals.turnTimer.turnStartTime) {
    timeleft = Math.ceil(
      (c.CONSTANTS.TIME_TURN_MS - (Date.now() - globals.turnTimer.turnStartTime)) /
      1000,
    );
...
function drawAOEtips(aoes) {
  ...
  let posy = buttonSpell.h_offset - buttonSpell.height - 20 - aoes.length * 60 -10;
  ctx.beginPath();
  ctx.fillRect(
    posx,
    posy,
