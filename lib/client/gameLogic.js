...
function onMouseClicHUD(x, y) {
    ...
    for (let i = 0; i < 3; i++) {
        const btnX = buttonSpell.w_offset + i * (buttonSpell.width + 10);
        const btnY = buttonSpell.h_offset - buttonSpell.height;
    ...
    let btnX = c.CANVAS.WIDTH * 7/ 10
    let btnY = buttonSpell.h_offset - buttonSpell.height;

    console.log(x,btnX, btnX + buttonSpell.width)
    if (x > btnX && x < btnX + buttonSpell.width && y > btnY && y < btnY + buttonSpell.height) {
        console.log("click rise lava ")
        clickRiseLava()
        return true;
    }
    return false
