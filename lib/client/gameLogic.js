...
function listenToMouse() {
    ...
    canvas.addEventListener('click', function (event) {
        ...
        console.log("reason for passing : cast rise lava")
                    turnOrder.passTurn();
                }
            }
            modeClic = "MOVE"
...
function castSpell(caster, spell, cell, direction) {
    ...
    if (utils.findPlayerFromEntity(caster).dead){
        console.log("reason for passing : caster just suicided by casting spell ",spell.name)
        turnOrder.passTurn();
    }
}
...
function clickPassTurn() {
    console.log("reason for passing : click on pass")
    ...
    if (action) {
        ...
        if (found) {
            ...
            if (action.kind == "MOVE") {
                ...
                castSpell(currentPlayer.entity, c.GAMEDATA.LAVA_SPELL, found,)
        console.log("reason for passing : recieved lava signal")
                turnOrder.passTurn();
            }
        }
        else if (action.kind == "PASS") {
        console.log("reason for passing : recieved pass signal")
            turnOrder.passTurn();
