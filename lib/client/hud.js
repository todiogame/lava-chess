...
function displayProfiles(me,enemy) {
  ...
  document.getElementsByClassName("profiles")[1].style.color = TEAM == c.CONSTANTS.TEAM_BLUE ? c.CONSTANTS.TEAM_RED : c.CONSTANTS.TEAM_BLUE;
  ...
  if (player) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (spell) {
        ...
        button.getElementsByClassName('content')[0].textContent = `${spell.name} ` +(spell.cooldown ? `${'CD: '+(spell.currentCD > 0 ? spell.currentCD+'/' : '')} ${spell.cooldown}` : ``);
        button.getElementsByClassName('tooltip')[0].textContent = spell.description ? `${spell.description}` : "";
...
function displayPickCharacterHUD() {
  ...
  document.getElementById("picking-team").textContent = ((currentTeam == TEAM) ? "Your turn" : "Enemy's turn") + " to "
    + (pickOrBanIndex < c.CONSTANTS.PICK_BAN_ORDER.length ? c.CONSTANTS.PICK_BAN_ORDER[pickOrBanIndex] : "...");
  ...
  if (currentEntity) {
    ...
    document.getElementById("picking-difficulty").textContent = "Difficulty: "+displayStars(currentEntity.difficulty);
    document.getElementById("picking-description").textContent = currentEntity.description;
    ...
    for (let i = 0; i < currentEntity.spellsDisplay.length; i++) {
      ...
      if (spell) {
        ...
        divSpell.getElementsByClassName('content')[0].textContent = `${spell.name} `
...
module.exports = {
  displayProfiles, displayCharacterHUD, switchToGameMode, displayPickCharacterHUD
};