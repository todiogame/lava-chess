...
function playAction(action, og) {
  if (action) {
    ...
    console.log(utils.findEntityById(action.entityId, og))
    let found;
    if (action.cell) found = og.map.find((b) => b.equals(action.cell));
    if (found) {
      if (action.kind == "PICK") {
        passTurnPick(utils.findEntityById(action.entityId, og), found.copy());
      } else if (action.kind == "BAN") {
        passTurnBan(utils.findEntityById(action.entityId, og), found.copy());
      }
    }
  }
}
...
function initPickPhase() {
  ...
  console.log(og.entities)
  drawing.loadIconsForDraft(og.entities)
}
module.exports = {
  initPickPhase,
  onMouseClicDraft,
