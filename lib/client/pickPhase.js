...
function onMouseClicDraft(hexagon) {
    ...
    if (TEAM == currentTeam) {
        ...
        if (found) {
            ...
            //pick
            if (c.CONSTANTS.PICK_BAN_ORDER[pickOrBanIndex] == "PICK" &&
                entities.find(e => e.selected) && utils.isSpawn(found, TEAM) && utils.isFree(found) && found.floor) {
            ...
            // ban 
            if (c.CONSTANTS.PICK_BAN_ORDER[pickOrBanIndex] == "BAN" &&
                entities.find(e => e.selected) && utils.isBanArea(found)) {
                let selectedEnt = entities.find(e => (e.selected));
...
function passTurnBan(entity, cell) {
    ...
    pickOrBanIndex++
    console.log("now " + currentTeam + "'s turn to " + c.CONSTANTS.PICK_BAN_ORDER[pickOrBanIndex])
}
