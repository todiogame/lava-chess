...
module.exports = class Game {
  constructor(clientA, clientB, gameMode, gameId) {
    ...
    // this.finished = false;
    // this.turnTimer = {};
    // this.map = logic.initMap(c.CONSTANTS.MAP_RADIUS)

    // this.og.currentPlayer = this.PLAYERS[0]
    ...
    if (gameMode == c.GAME_MODE.DRAFT)
      ...
      this.og = new OngoingGame(players)
    ...
    if (action) {
      ...
      if (found) {
        ...
        if (action.kind == "MOVE") {
          ...
          logic.castSpell(this.og, this.og.currentPlayer.entity, this.og.currentPlayer.spells[action.spellID], found, action.direction);

        }
        else if (action.kind == "LAVA") {
          logic.castSpell(this.og, this.og.currentPlayer.entity, c.GAMEDATA.LAVA_SPELL, found,)
      ...
      utils.checkWinCondition(this.og, this)
    }
  }
