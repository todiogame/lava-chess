...
function damage_others(cell, spell, casterEntity, targetEntity) {
  ...
  if (summonData && !targetEntity) {
    ...
    if (summoned.auras)
      summoned.auras.forEach((a) => {
        a.source = currentPlayer;
        if (typeof window != 'undefined' && window.document) {
          if (summonData.src) {
            let imageAura = new Image();
            summonData.src = a.src;
            a.image = imageAura
...
function zombie_crawl(cell, spell, casterEntity, targetEntity) {
  ...
  if (targetEntity) {
    ...
    if (utils.isEntityAlive(casterEntity)) casterEntity.die();
  }
  else {
    if (typeof window != 'undefined' && window.document)
...
function gasser_shotgun(cell, spell, casterEntity, targetEntity) {
  damage(cell, spell, casterEntity, targetEntity);
  let gas = {};
  Object.assign(gas, utils.findCharacterFromName("Gasser").spells[0]);
  gas.source = utils.findPlayerFromEntity(casterEntity);
  if (cell.floor && !cell.aoe.find((s) => s.name == gas.name))
    cell.aoe.push(gas);
}

function lava_flow(cell, spell, casterEntity, targetEntity) {
  let pos = casterEntity.pos;
  blink(cell, spell, casterEntity, targetEntity);
  riseLava(pos, spell, casterEntity, null);
}
