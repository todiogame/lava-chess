...
function damage(cell, spell, casterEntity, targetEntity) {
    if (targetEntity && !targetEntity.isInvulnerable) {
        ...
        Anim.damageParticles(targetCell)
...
function damage_others(cell, spell, casterEntity, targetEntity) {
    if (targetEntity != casterEntity) damage(cell, spell, casterEntity, targetEntity);
}
...
function pango_spiky_ball(cell, spell, casterEntity, targetEntity) {
    ...
    let newAura =  { name: spell.name, dealSpell: "damage", aoe: "ring_1+diag", isAura: true, glyph: 1, color: "GLYPH_BROWN", description: "Deals damage around the caster." }
